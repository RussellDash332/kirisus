...
GROUP_CHATS = [CLUSTER_CHAT, PCT_CHAT]
...
def mahjong(message):
    if message.chat.id in [*GROUP_CHATS, SL_CHAT]: return
...
def pinned(message):
    if message.chat.id in GROUP_CHATS and message.chat.username != RUSSELL:
        # if triggered in a group chat, only Russell can do it
        return
